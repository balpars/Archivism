...
def transcribe_audio(file_path: str, output_folder: str, language: str = None, model_size: str = 'turbo', progress_callback=None) -> float:
    try:
        ...
        # Create a text-only transcription file
        text_file = Path(output_folder) / "transcription.txt"
        with text_file.open('w', encoding='utf-8') as f:
            # Join all segment texts with a newline
            transcription_text = "\n".join(segment['text'] for segment in result['segments'])
            f.write(transcription_text)

        save_transcription_as_word(result, output_folder, "transcription", include_timestamps=False)
